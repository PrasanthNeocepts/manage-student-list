...
server.post("/students/", function(req, res, next) {
  ...
  if (error) {
    ...
    req.body.studentName = createSlug(req.body.studentName); // Generate a slug for new students.
    next();
  }
});
...
function validatestudent(student) {
  if (!student.studentName) return "studentName is required.";
  if (!student.age) return "age is required.";
  if (!student.email) return "email is required.";
  if(!student.phnNumber) return "phnNumber is required.";
  if(!student.branchId) return "branch is required.";
  return "";
}
